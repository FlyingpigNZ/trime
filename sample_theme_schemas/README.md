...
- `component.yaml` + `keyboard.yaml` + `behavior.yaml` + `color.yaml` +
  `style.yaml` — component-model authoring source
...
- **App-shipped assets** (`app/src/main/assets/shared/`): standard catalog,
  `shared-aux`, and built-in themes (e.g. tongwenfeng).
- **Customer-defined input method packages** (e.g. 简纯+14键 / 14jian) are
