...
- 小鹤 zero-initial syllables (a, ai, an, ang, ao, e, ei, en, eng, er, o, ou)
  ...
  `ou` — the algebra drops the guide+key forms `^(aj|ac|ad|ew|ef|oz)`),
  single-letter finals double (`a → aa`, `o → oo`, `e → ee`) and longer finals
  take a vowel guide + final key (`ang → ah`, `eng → eg`); never the bare
  final key alone (`ang → h`).
