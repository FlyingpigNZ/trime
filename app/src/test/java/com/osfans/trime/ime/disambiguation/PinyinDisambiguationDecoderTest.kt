...
class PinyinDisambiguationDecoderTest :
    StringSpec({

        fun full(syllables: List<SchemaExtension.T9Disambiguation.Syllable>) = PinyinDisambiguationDecoder(
        ...
        fun flypy(syllables: List<SchemaExtension.T9Disambiguation.Syllable>) = PinyinDisambiguationDecoder(
            SchemaExtension.T9Disambiguation(
                enabled = true,
        ...
        fun flypy14(syllables: List<SchemaExtension.T9Disambiguation.Syllable>) = PinyinDisambiguationDecoder(
            SchemaExtension.T9Disambiguation(
                enabled = true,
                inputMethod = SchemaExtension.T9Disambiguation.InputMethod.FLYPY14,
                syllables = syllables,
            ),
        )
        ...
        val flypy14Sample = listOf(
            // a/sa: 双拼 aa/sa both fold to the typed letters `aa`
            syl14("a", "aa", "aa"),
            syl14("sa", "sa", "aa"),
            // zhong/ca: 双拼 vs/ca both fold to the typed letters `ca`
            syl14("zhong", "vs", "ca"),
            syl14("ca", "ca", "ca"),
        )

        "flypy14: decodeLeading shows every syllable whose 双拼 code folds to the typed letters" {
            flypy14(flypy14Sample).decodeLeading("aa").map { it.pinyin[0] to it.code } shouldContainExactly
                listOf("sa" to "sa", "a" to "aa")
            flypy14(flypy14Sample).decodeLeading("ca").map { it.pinyin[0] to it.code } shouldContainExactly
                listOf("zhong" to "vs", "ca" to "ca")
        }

        "flypy14: decode pairs two-letter syllables across the input" {
            flypy14(flypy14Sample).decodeToDisplay("aaca") shouldContainExactlyInAnyOrder listOf(
                "a zhong",
                "a ca",
                "sa zhong",
                "sa ca",
