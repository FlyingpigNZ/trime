...
/**
 * Decodes a lossy key-code string into the set of legal pinyin sequences.
 *
 * Feature ②: the user types a key code whose letters/digits are folded (T9
 * digit keys, or the 小鹤双拼14键 letter keys); this decoder lists every legal
 * pinyin parse of the code string so the app can show them in the overlay
 * strip (T9 first column / 14-key first row) and feed the chosen pinyin back
 * to Rime.
 *
 * Three paths:
...
class PinyinDisambiguationDecoder(
    ...
    /** Complete-syllable matches for a code prefix: (pinyin, rime code). */
    ...
    // ── flypy / flypy14: two code characters per syllable ─────────────────
    private fun decodeFlypy(
        code: String,
        syllablesByCode: Map<String, List<SchemaExtension.T9Disambiguation.Syllable>>,
    ): List<List<String>> {
        ...
        fun dfs(
            ...
            val pair = code.substring(start, start + 2)
            val syllables = syllablesByCode[pair] ?: return
